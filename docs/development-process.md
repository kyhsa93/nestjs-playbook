...
| 레거시 기능 수정 | DM → TD → **(사용자 확인)** → TE → IM → VA | 레거시 코드에서 기능 수정 시, 해당 Vertical Slice를 가이드에 맞춰 동시 리팩토링 |
...
1. 기능 수정 요청 접수
2. 해당 기능의 현재 코드 경로 파악 (어떤 레이어를 거치는지)
3. DM: Slice 범위 내 도메인 모델을 가이드 기준으로 재설계
